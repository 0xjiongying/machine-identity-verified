...
| CCP approval | `POST /verify_apass` → `data.code === 4` only |
...
\* Sandbox accepts apply (`requestId`) but Monad custom launch returns `ISSUE_FAILED`. Pipeline binds registered Monad **aUSDC** `0xaC0893567D43C3E7e6e35a72803df05416C1f20D` for CCP — never fabricates ISSUED.
